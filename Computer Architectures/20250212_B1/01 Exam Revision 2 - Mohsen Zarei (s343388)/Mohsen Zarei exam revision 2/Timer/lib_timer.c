...
// <<< end of configuration section >>>
	...
	/*NVIC_SetPriority(TIMER0_IRQn, 4);*/		/* less priority than buttons */ //priorities need to not be the same
...
void delay_ms(uint32_t ms){
  ...
  uint32_t startTicks = tick;
  while((tick - startTicks) < ms);	
